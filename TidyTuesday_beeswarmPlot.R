...
final <- bechdel_beeswarm / bechdel_stream / bechdel_genre +
  ...
  plot_annotation(
    ...
    theme = theme(
      ...
      plot.background = element_rect(fill = bck_clr, color = NA),
      plot.title = element_text(family = "oswald", size = 10, hjust = 0.9, margin = margin(15,0,0,0), color = "white"),
      plot.subtitle = element_text(family = "roboto", size = 8, hjust = 0.9, margin = margin(10,0,0,0), lineheight = 1.1),
      plot.caption = element_text(family = "techmono", size = 5, color = "white", margin = margin(15,0,0,0))      
    )
  )
